...
from protosc.model.genetic import Chromosome
...
def test_chromosome(n_samples, n_features):
    n_true_features = n_features // 2
    X, y, _ = create_independent_data(n_samples=n_samples,
...
def test_ga(n_samples, n_features, n_jobs):
    n_true_features = n_features // 2
    X, y, _ = create_independent_data(
        n_samples=n_samples, n_features=n_features,
        n_true_features=n_true_features)

    features = GeneticModel(n_random_features=100).execute(X, y, n_jobs=n_jobs)
#     print(X_gen.shape)
