...
def sort_chisquare(X_chisquare):
    ...
    chisquare_sorted = chisquare_sorted.sort_values(
        by='chisquare', ascending=False).reset_index(drop=True)

    return chisquare_sorted


def cumsum_chisquare(chisquare_sorted):
    """ Calculated the cumulative sum of the chi-sqaure vector and normalize to 1 """
